...
CREATE TABLE orders (id SERIAL PRIMARY KEY, user_id INT references users(id), amount INT, order_number INT);

CREATE TABLE totals (id SERIAL PRIMARY KEY, order_id INT references orders(id), item_id INT references items(id));
